...
export function useLoginStyles() {
  ...
  // 液态玻璃背景层
  // ============================================================
  const bgLayerClassName = computed(() => cn('absolute inset-0 -z-10 overflow-hidden pointer-events-none'))
  ...
  const brandContentClassName = computed(() => cn('relative z-10 w-full max-w-[340px] mx-auto', 'flex flex-col gap-8'))

  /** 产品预览卡（更精细的 dashboard 缩略图） */
  const brandPreviewClassName = computed(() =>
    cn('relative rounded-xl overflow-hidden', 'bg-white/[0.08] backdrop-blur-sm', 'border border-white/[0.12]', 'p-3'),
  )
  ...
  /** 数据指标容器 */
  const brandStatsClassName = computed(() => cn('grid grid-cols-3 gap-3', 'pt-6', 'border-t border-white/10'))

  /** 单个数据指标 */
  const brandStatItemClassName = computed(() => cn('flex flex-col gap-0.5'))

  /** 数据数字 */
  const brandStatValueClassName = computed(() => cn('text-lg font-semibold text-white tracking-tight'))

  /** 数据标签 */
  const brandStatLabelClassName = computed(() => cn('text-[10px] text-white/50 uppercase tracking-wider'))
  return {
    containerClassName,
    bgLayerClassName,
    blob1ClassName,
