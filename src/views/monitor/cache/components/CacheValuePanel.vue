...
async function loadValue() {
  ...
  loading.value = true;
  try {
    const res = (await getCacheValue({ key: props.cacheKey })) as
      | { data?: CacheKeyValue }
      | CacheKeyValue;
      ...
      <div v-else-if="detail" class="space-y-3 p-3">
        ...
        <div class="grid grid-cols-3 gap-2 text-xs">
          ...
          >
            ...
            <span class="break-all font-mono text-gray-800 dark:text-gray-100">
              {{ detail.key }}
            </span>
          </div>
          <div
            class="rounded border border-gray-100 bg-gray-50 px-2 py-1.5 dark:border-gray-800 dark:bg-gray-800/40"
          >
        ...
        <div class="rounded border border-gray-100 dark:border-gray-800">
          <div
            class="flex items-center justify-between border-b border-gray-100 px-2 py-1.5 dark:border-gray-800"
          >
            <span class="text-xs font-medium text-gray-600 dark:text-gray-300"> 值 </span>
            <div class="flex items-center gap-2">
              <span v-if="detail.truncated" class="text-[10px] text-amber-500 dark:text-amber-400">
                已截断
              </span>
              <button
                type="button"
