...
import { jobActionColumn, jobColumns, jobRowKey, jobScroll, logColumns, logRowKey } from './columns'
...
<template>
  <a-card :bordered="false" class="shadow-sm">
    <a-tabs v-model:active-key="activeTab">
      <a-tab-pane key="job" tab="任务列表" :disabled="!hasPermission('monitor:job:list')">
        <BasicTable
          :columns="jobColumns"
          :api="getJobList"
          :immediate="true"
        ...
        >
          <template #toolbar>
            <a-button type="primary" @click="handleAdd()" v-permission="'monitor:job:create'">
          ...
          <template #cell-status="{ record }">
            <a-switch :checked="record.status === '1'" size="small" @change="() => handleToggle(record as JobRecord)" />
          </template>
      ...
      <a-tab-pane key="log" tab="执行日志" :disabled="!hasPermission('monitor:job:log')">
        <BasicTable
          :columns="logColumns"
          :api="getJobLogList"
          :immediate="true"
    ...
    <BasicModal :title="isEditing ? '编辑任务' : '新增任务'" :width="620" @register="modalRegister" @ok="handleSave">
      <BasicForm
