...
import { type AccessibleTenant, changePassword, getMyTenants, switchTenant, updateProfile } from '~/api/auth'
import { disableMFA, enableMFA, getMFAStatus, type MFAStatus, regenerateBackupCodes, setupMFA } from '~/api/mfa'
...
function handleRegenerateBackupCodes() {
  openTokenModal('重新生成备份码', '生成后旧的备份码将失效，请输入当前 6 位验证码', async (token) => {
...
<template>
  <div :class="containerClassName">
    <!-- ==================== 用户卡片 ==================== -->
    <div class="overflow-hidden rounded-xl border border-gray-200/70 bg-white dark:border-gray-800 dark:bg-gray-900">
      <div class="relative flex items-center gap-6 px-6 py-6">
        <a-avatar
          :size="80"
          :src="userStore.avatar"
          class="bg-slate-100 text-2xl text-slate-600 ring-2 ring-gray-100 dark:bg-slate-800 dark:text-slate-300 dark:ring-gray-800"
        >
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="mt-1 text-sm text-gray-500 dark:text-gray-400">@{{ userStore.username }}</div>
            ...
            >
              ...
              <template #actionAfter>
                <div class="mt-4 flex justify-end">
                  <a-button type="primary" :loading="profileLoading" @click="handleSaveProfile"> 保存修改 </a-button>
                </div>
              </template>
            </BasicForm>
          </div>
            ...
            <div>
              ...
              <div class="flex items-center justify-between rounded-lg border border-gray-100 p-4 dark:border-gray-800">
                ...
                <div class="flex gap-2">
                  <a-button v-if="!mfaStatus.enabled" type="primary" :loading="mfaLoading" @click="handleMFASetup">
                    启用 MFA
                  </a-button>
                  <template v-else>
    ...
    >
      <div v-if="mfaSetupData" class="space-y-3">
        <p class="text-sm text-gray-500">使用验证器 App（如 Google Authenticator）扫描下方二维码：</p>
        <div class="flex justify-center">
          <img :src="mfaSetupData.qrCode" alt="QR" class="h-40 w-40" />
        </div>
        <div class="text-center text-xs text-gray-500">
          无法扫描？手动输入：<code>{{ mfaSetupData.manualEntryKey }}</code>
        </div>
        <a-input v-model:value="mfaToken" placeholder="请输入 6 位验证码" :maxlength="6" size="large" />
      </div>
    </a-modal>
    ...
    <!-- ==================== 备份码弹窗 ==================== -->
    <a-modal :open="backupCodes.length > 0" title="备份码（请妥善保存）" :footer="null" @cancel="backupCodes = []">
      <div class="grid grid-cols-2 gap-2">
        <code
          v-for="c in backupCodes"
          :key="c"
          class="rounded bg-gray-50 px-2 py-1 text-center text-xs dark:bg-gray-800"
        >
          {{ c }}
        </code>
      </div>
      <div class="mt-3 text-xs text-gray-400">每个备份码只能使用一次，请离线保存到安全位置。</div>
    </a-modal>
  </div>
</template>
