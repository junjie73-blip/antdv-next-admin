    ...
    <div :class="statsGridClassName">
      ...
      <a-card :class="cardClassName" size="small">
        <a-statistic title="本周登录次数" :value="weekLoginCount" suffix="次">
          <template #prefix>
            <Icon icon="carbon:calendar" class="text-green-500 text-lg mr-1" />
          </template>
        </a-statistic>
      </a-card>
      <a-card :class="cardClassName" size="small">
      ...
      <a-card :class="[cardClassName, failCardClassName]" size="small">
        ...
        >
          <template #prefix>
            <Icon icon="carbon:error" class="text-red-500 text-lg mr-1" />
          </template>
        </a-statistic>
      </a-card>
    </div>
        ...
        <template #cell-status="{ record }">
          <a-tag :color="record.status === 'success' ? 'green' : 'red'">
            <span :class="tagClassName">
              <Icon
                :icon="
                  record.status === 'success' ? 'carbon:checkmark-outline' : 'carbon:close-outline'
                "
              />
