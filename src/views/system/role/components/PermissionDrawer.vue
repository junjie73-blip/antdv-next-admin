...
<template>
  <a-drawer :open="open" :title="`权限分配 - ${role?.roleName || ''}`" :width="560" :footer="null" @close="close">
    <div class="flex h-full flex-col">
          ...
          <a-tabs v-model:active-key="activeTab" class="h-full">
            ...
            <a-tab-pane key="api" tab="API 权限">
              <a-spin :spinning="apiLoading">
                ...
                <a-empty v-if="apiOptions.length === 0 && !apiLoading" description="暂无可分配权限" />
              </a-spin>
            </a-tab-pane>
            ...
            <a-tab-pane key="users" tab="关联用户">
              <a-spin :spinning="userLoading">
                <a-select
                  ...
                  show-search
                  :filter-option="
                    (input: string, opt: any) => String(opt.label).toLowerCase().includes(input.toLowerCase())
                  "
                  class="w-full"
                />
              </a-spin>
            ...
            <a-tab-pane v-if="role?.dataScope === '2'" key="dept" tab="数据权限">
              <a-alert message="该角色数据权限为「自定义」，请勾选可见部门" type="warning" show-icon class="mb-3" />
              <a-tree
                checkable
                default-expand-all
      ...
      <div class="flex justify-end gap-2 border-t border-solid border-gray-200 pt-4">
