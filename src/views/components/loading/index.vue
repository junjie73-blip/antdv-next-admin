  ...
  <div :class="containerClassName">
    <a-card :variant="borderless" title="Spin 加载器">
      <div :class="cn('space-y-8')">
        <div>
          <h4 class="mb-3 text-base font-medium text-gray-700 dark:text-gray-300">基础用法</h4>
          <div class="flex flex-wrap items-center gap-8">
            <a-spin />
            <a-spin :loading="spinLoading" description="数据加载中..." />
            <a-button @click="spinLoading = !spinLoading"> {{ spinLoading ? '停止' : '开始' }}加载 </a-button>
          </div>
        </div>

        <div>
          <h4 class="mb-3 text-base font-medium text-gray-700 dark:text-gray-300">包裹内容</h4>
          <div class="flex flex-wrap gap-6">
            <a-spin :loading="true" class="w-[240px]">
              <div
            ...
            <a-spin :loading="spinLoading" class="w-[240px]">
        ...
        <div>
          <h4 class="mb-3 text-base font-medium text-gray-700 dark:text-gray-300">自定义提示文字</h4>
          <div class="flex flex-wrap items-center gap-4">
            <a-spin description="正在加载用户信息..." />
            <a-spin description="请稍候，数据同步中...">
              <div class="h-[80px] w-[200px] rounded-lg bg-blue-50 p-3 dark:bg-blue-900/20" />
            </a-spin>
            <a-input v-model:value="spinTip" placeholder="输入提示文字" style="width: 180px" />
            <a-spin :description="spinTip" />
          </div>
        </div>

        <div>
          <h4 class="mb-3 text-base font-medium text-gray-700 dark:text-gray-300">尺寸大小</h4>
        ...
        <div>
          <h4 class="mb-3 text-base font-medium text-gray-700 dark:text-gray-300">样式切换（点状 / 圆圈 / 网格）</h4>
          <div class="flex flex-wrap items-center gap-6">
            ...
            <div
              class="ml-4 flex min-h-[100px] min-w-[160px] items-center justify-center rounded-lg bg-gray-50 p-4 dark:bg-gray-800"
            >
              <a-spin v-if="spinStyle === 'circle'" />
              <a-spin v-else-if="spinStyle === 'dot'" dot />
              <a-spin v-else-if="spinStyle === 'grid'" grid />
            </div>
          </div>
        </div>
      </div>
    ...
    <a-card :variant="borderless" title="Skeleton 骨架屏">
      <div :class="cn('space-y-8')">
        <div>
          <h4 class="mb-3 text-base font-medium text-gray-700 dark:text-gray-300">头像 + 文本骨架</h4>
          <div class="flex gap-8">
            <a-skeleton avatar :paragraph="{ rows: 3 }" />
            <a-skeleton avatar active :paragraph="{ rows: 2 }" />
          </div>
        </div>

        <div>
          <h4 class="mb-3 text-base font-medium text-gray-700 dark:text-gray-300">图片骨架</h4>
          <div class="flex flex-wrap gap-6">
            <a-skeleton-image />
        ...
        <div>
          <h4 class="mb-3 text-base font-medium text-gray-700 dark:text-gray-300">表格骨架</h4>
          <a-skeleton-table :columns="5" :rows="4" />
        </div>

        <div>
          <h4 class="mb-3 text-base font-medium text-gray-700 dark:text-gray-300">按钮骨架</h4>
        ...
        <div>
          <h4 class="mb-3 text-base font-medium text-gray-700 dark:text-gray-300">动画控制</h4>
          <div class="flex items-center gap-4">
            <a-switch v-model:checked="skeletonActive" checked-children="激活" un-checked-children="静止" />
            <span class="text-sm text-gray-500">
              当前状态：<span class="font-medium">{{ skeletonActive ? '流动动画' : '静止' }}</span>
            </span>
          </div>
          <div class="mt-4 max-w-md rounded-lg bg-gray-50 p-4 dark:bg-gray-800">
            <a-skeleton :active="skeletonActive" :paragraph="{ rows: 2 }" />
          </div>
        </div>
      </div>
    </a-card>
      ...
      <div :class="cn('space-y-6')">
        ...
        <div class="relative overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
          <div
            ...
            class="min-h-[180px] bg-gradient-to-br from-blue-50 to-indigo-50 p-6 dark:from-gray-800 dark:to-gray-900"
          >
            <div class="text-center">
              <p class="mb-4 text-sm text-gray-500 dark:text-gray-400">这是一个容器内 Loading 演示区域</p>
              <a-button type="primary" :loading="componentLoading" @click="triggerComponentLoading">
    ...
    <a-card :variant="borderless" title="useLoading Hook">
      <div :class="cn('space-y-4')">
        <p class="text-sm text-gray-500 dark:text-gray-400">
          通过
          <code class="rounded bg-gray-100 px-1.5 py-0.5 text-xs dark:bg-gray-800">useLoading</code>
        ...
        <div class="relative overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
          <div
            ref="hookContainerRef"
            class="flex min-h-[160px] items-center justify-center bg-gradient-to-br from-emerald-50 to-teal-50 p-6 dark:from-gray-800 dark:to-gray-900"
          >
            <div class="text-center">
              <p class="mb-4 text-sm text-gray-500 dark:text-gray-400">useLoading 控制此容器的加载状态</p>
              <a-button type="primary" status="success" @click="triggerUseLoading"> 触发 useLoading </a-button>
            </div>
          </div>
        </div>

        <div class="rounded-lg bg-gray-50 p-4 dark:bg-gray-800/60">
          <pre class="font-mono text-xs leading-relaxed whitespace-pre-wrap text-gray-600 dark:text-gray-400">
const hookLoading = useLoading({
  target: containerRef,
  body: false,
  tip: '加载中...'
...
hookLoading.setTip('新提示文字') // 修改提示</pre>
    ...
    <a-card :variant="borderless" title="按钮加载态">
      <div :class="cn('space-y-6')">
        <p class="text-sm text-gray-500 dark:text-gray-400">
          Antdv-next 按钮内置
          <code class="rounded bg-gray-100 px-1.5 py-0.5 text-xs dark:bg-gray-800">loading</code>
          属性，点击后自动进入加载态并禁用交互
        </p>

        <div class="flex flex-wrap gap-4">
          <a-button type="primary" :loading="btnLoading1" @click="triggerBtnLoading(0)"> 主要按钮 </a-button>
          <a-button :loading="btnLoading2" @click="triggerBtnLoading(1)"> 默认按钮 </a-button>
          <a-button type="dashed" :loading="btnLoading3" @click="triggerBtnLoading(2)"> 虚线按钮 </a-button>
          <a-button type="outline" :loading="btnLoading1" disabled> 禁用按钮 </a-button>
        </div>

        <div class="flex flex-wrap gap-4">
          <a-button type="primary" loading> 常驻加载态 </a-button>
          <a-button type="primary" loading> 提交中... </a-button>
          <a-button type="primary" shape="round" :loading="btnLoading2" @click="triggerBtnLoading(1)">
            圆角按钮
          </a-button>
        </div>
      </div>
    ...
    <a-card :variant="borderless" title="全局加载遮罩">
      <div :class="cn('space-y-4')">
        <p class="text-sm text-gray-500 dark:text-gray-400">
          使用
          <code class="rounded bg-gray-100 px-1.5 py-0.5 text-xs dark:bg-gray-800">createLoading</code>
          创建全屏加载实例， 适用于路由跳转、API 请求拦截器等场景。支持动态修改提示文字
        </p>

        <div class="flex gap-4">
          <a-button type="primary" status="danger" @click="triggerGlobalLoading"> 模拟 API 请求 (全屏遮罩) </a-button>
        </div>
