...
<template>
  <div :class="containerClassName">
    ...
    <BasicModal @register="registerBasicModal" title="基础弹窗" @ok="handleSubmit">
    ...
    <BasicModal @register="registerLargeModal" title="大尺寸弹窗" width="800px" ok-text="确认" cancel-text="取消">
    ...
    <BasicModal @register="registerNoFooterModal" title="无底部按钮弹窗" :footer="null">
      <p class="text-gray-600 dark:text-gray-400">此弹窗没有底部按钮。点击 X 按钮关闭。</p>
    </BasicModal>

    <BasicModal @register="registerAsyncModal" title="异步加载数据" @ok="asyncModalMethods?.closeModal()">
      ...
      <div v-else class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
        ...
        <p class="text-blue-600 dark:text-blue-400">{{ asyncResult }}</p>
      </div>
    </BasicModal>

    <BasicModal @register="registerContentModal" v-model:open="contentModalVisible" title="Content Modal">
      <div class="space-y-4">
        <div class="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <h3 class="font-semibold text-gray-800 dark:text-gray-200 text-lg mb-2">{{ contentData.title }}</h3>
          <p class="text-gray-600 dark:text-gray-400">{{ contentData.description }}</p>
        </div>
