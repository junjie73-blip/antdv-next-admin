    ...
    <a-card title="基础表格">
      <BasicTable @register="registerBasic">
        <template #cell-status="{ record }">
          <a-tag
            :color="
              record?.status === '在职' ? 'green' : record?.status === '休假' ? 'blue' : 'default'
            "
          >
            {{ record?.status }}
          </a-tag>
...
@click="handleRefresh"> 刷新数据 </a-button>
      </div>
    </a-card>

    <a-card title="骨架屏加载动画">
      <a-alert message="点击下方按钮查看加载状态"
type="info"
show-icon
:class="infoClassName" />
...
:paragraph="{ rows: 5 }">
      ...
      <div :class="toolbarClassName">
        <a-button :type="loading ? 'default' : 'primary'"
