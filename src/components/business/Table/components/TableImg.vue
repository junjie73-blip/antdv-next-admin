...
<template>
  <Space :size="margin">
    <Image
      ...
      :class="
        cn(
          'rounded border border-gray-200 cursor-pointer hover:border-blue-500 hover:shadow-md transition-all',
        )
      "
    />
    <span v-if="simpleShow && imgList && imgList.length > 1" :class="cn('text-gray-400 text-sm')">
      +{{ imgList.length - 1 }}
    </span>
  </Space>
</template>
