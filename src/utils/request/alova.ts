...
const RETRYABLE_STATUS_CODES = new Set([408, 429, 500, 501, 502, 503, 504, 505, 507, 508, 510, 511])
    ...
    async beforeRequest(method) {
      ...
      if (!isAuthApi) {
        ...
        // ---------- 2.2 access token 快过期 ----------
          ...
          if (refreshToken) {
            try {
              console.log('[Auth] accessToken 即将过期（剩余', getTokenRemainingSeconds(accessToken), '秒），主动刷新')
              await getRefreshPromise()
            } catch (err) {
      ...
      async onSuccess(response, method) {
        ...
        if (responseType === 'blob' || responseType === 'arrayBuffer') {
          ...
          const err = new AlovaRequestError(
            resolveErrorMessage(errorPayload, `${response.status} ${response.statusText}`, response.status, bodyCode),
            {
              data: errorPayload,
              status: response.status,
        ...
        if (!response.ok) {
          const err = new AlovaRequestError(
            resolveErrorMessage(payload, `${response.status} ${response.statusText}`, response.status, bodyCode),
            {
              data: payload,
              status: response.status,
              statusText: response.statusText,
        ...
        if (bodyCode !== undefined && bodyCode !== ErrorCode.SUCCESS) {
          const err = new AlovaRequestError(resolveErrorMessage(payload, '请求失败', response.status, bodyCode), {
            data: payload,
            status: response.status,
            statusText: response.statusText,
            code: bodyCode,
...
function resolveErrorMessage(data: unknown, fallback: string, status?: number, code?: number): string {
